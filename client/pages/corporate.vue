<template>
	<section>
		<div class="main-form bg-office">
			<div class="container">
				<h1 v-html="$t('corp.title')" class="home-title m-0"></h1>
				<p v-html="$t('corp.subtitle')" class="home-subtitle"></p>
		...
		<modal :classes-modal="['modal', 'modal-form']" :action="signUp" :title="$t('menu.corporate')" id="business-corp"
		       :inputs="inputs" :show-modal="showModal"></modal>
...
export default {
	...
	methods: {
		...
		async signUp(inputs, modal, event) {
			...
			await this.$store.dispatch('message/SEND_MESSAGE_AS_CORPORATE', payload);
