...
export default {
  ...
  async asyncData({app}) {
    await app.store.dispatch('panel/user/GET_USERS', '?page=1&limit=10');
    const users = await app.store.getters['panel/user/GET_USERS']
    const roles = await app.store.getters['panel/user/GET_ROLES']
    let totalNumberOfItems = await app.store.getters['panel/user/GET_TOTAL']

    return {users, totalNumberOfItems, roles}
  },
