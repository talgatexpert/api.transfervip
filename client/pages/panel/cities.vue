...
export default {
  ...
  async asyncData({app}) {
    await app.store.dispatch('panel/city/GET_CITIES', '?page=1&limit=10&orderby=turkish');
    const cities = await app.store.getters['panel/city/GET_CITIES']
    let totalNumberOfItems = await app.store.getters['panel/city/GET_TOTAL']

    return {cities, totalNumberOfItems}
  },
