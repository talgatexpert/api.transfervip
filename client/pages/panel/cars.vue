...
export default {
  ...
  async asyncData({app}) {
    await app.store.dispatch('panel/car/GET_CARS', '?page=1&limit=10');
    const cars = await app.store.getters['panel/car/GET_CARS']
    let totalNumberOfItems = await app.store.getters['panel/car/GET_TOTAL']
    return {cars, totalNumberOfItems}
  },
